import { Button, Disclosure, DisclosureButton, DisclosurePanel, Menu, MenuButton, MenuItem, MenuItems } from '@headlessui/react'
import { Bars3Icon, BellIcon, XMarkIcon } from '@heroicons/react/24/outline'
import bookify from "../assets/logo.png"
import {Link, useLocation} from "react-router-dom"
import {GoogleAuthProvider,signInWithPopup,signInWithRedirect} from "firebase/auth"
import {useState, useEffect} from "react"
import {authGet} from "../firebase"
import {useStateValue} from "../StateProvider"
  


const navigation = [
  { name: 'Categories', href: '/categories', current: false },
  { name: 'Authors', href: '/authors', current: false },
  { name: 'My Library', href: '/library', current: false },
  { name: 'Wishlist', href: '/wishlist', current: false },
]

function classNames(...classes) {
  return classes.filter(Boolean).join(' ')
}

export default function Example() {
    const [isLog,SetIsLog]= useState(false)
    const [state,dispatch]= useStateValue()

    const [urlPath,setUrlPath]= useState("/")
    const location= useLocation()
    useEffect(()=>{
      // const url=new URL(window.location.href) 
      setUrlPath(location.pathname)
    },[location])

    const signIn=async (e)=> {
        const provider = new GoogleAuthProvider();
        try {
          await signInWithPopup(authGet, provider);
        } catch (error) {
          console.error('Error signing in with Google:', error);
        }
      }
      const signOut=async()=>{
        try{
            await authGet.signOut();
            dispatch({TYPE:"USER",payload:null})
        }catch(error){
        console.log(error)
      }}
      
  return (
    <Disclosure as="nav" className="bg-gray-800 fixed w-full top-0 z-50">
      <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
        <div className="relative flex h-16 items-center justify-between">
          <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
            {/* Mobile menu button*/}
            <DisclosureButton className="group relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white">
              <span className="absolute -inset-0.5" />
              <span className="sr-only">Open main menu</span>
              <Bars3Icon aria-hidden="true" className="block size-6 group-data-[open]:hidden" />
              <XMarkIcon aria-hidden="true" className="hidden size-6 group-data-[open]:block" />
            </DisclosureButton>
          </div>
          <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
            <div className="flex shrink-0 items-center">
                <Link to="/">
              <img
                alt="Your Company"
                src={bookify}
                className="h-8 w-auto cursor-pointer"
              />
              </Link>
            </div>
            <div className="hidden sm:ml-6 sm:block">
              <div className="flex space-x-4">
                {navigation.map((item,index) => {
                    const list=["My Library","Wishlist"]
                    if(!state?.user && list.includes(item.name)){
                        return ;
                    }
                    return (<Link to={item.href} key={index}>
                  <p
                    key={item.name}
                    aria-current={item.href== urlPath ? 'page' : undefined}
                    className={classNames(
                      item.href== urlPath ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white',
                      'rounded-md px-3 py-2 text-sm font-medium',
                    )}
                  >{item.name} 
                  </p>
                  </Link>)
                    })}
              </div>
            </div>
          </div>
          <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
            <button
              type="button"
              className="relative rounded-full bg-gray-800 p-1 text-gray-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800"
            >
              <span className="absolute -inset-1.5" />
              <span className="sr-only">View notifications</span>
              {/* <BellIcon aria-hidden="true" className="size-6" /> */}
            </button>

            {/* Profile dropdown */}
            {!state?.user?<Button onClick={signIn} className="bg-blue-600 p-1 px-2 rounded-md">Log In</Button>:
            <Menu as="div" className="relative ml-3">
              <div>
                {<MenuButton  className="relative flex rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                  <span className="absolute -inset-1.5" />
                  <span className="sr-only">Open user menu</span>
                  <img
                    alt=""
                    src={state?.user?.photoURL}
                    className="size-8 rounded-full"
                  />
                </MenuButton>}
              </div>
              <MenuItems
                transition
                className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black/5 transition focus:outline-none data-[closed]:scale-95 data-[closed]:transform data-[closed]:opacity-0 data-[enter]:duration-100 data-[leave]:duration-75 data-[enter]:ease-out data-[leave]:ease-in"
              >
                <MenuItem>
                  <a
                    href="#"
                    className="block px-4 py-2 text-sm text-gray-700 data-[focus]:bg-gray-100 data-[focus]:outline-none"
                  >
                    Your Profile
                  </a>
                </MenuItem>
                <MenuItem>
                <Link to="/createdlist">
                  <p
                    className="block px-4 py-2 text-sm text-gray-700 data-[focus]:bg-gray-100 data-[focus]:outline-none"
                  >
                    Your Creation
                  </p>
                  </Link>
                </MenuItem>
                <MenuItem>
                <Link to="/createdauthor">
                  <p
                    className="block px-4 py-2 text-sm text-gray-700 data-[focus]:bg-gray-100 data-[focus]:outline-none"
                  >
                    Your Authors
                  </p>
                  </Link>
                </MenuItem>
                <MenuItem>
                <Link to="/createdcategory">
                  <p
                    className="block px-4 py-2 text-sm text-gray-700 data-[focus]:bg-gray-100 data-[focus]:outline-none"
                  >
                    Your Categories
                  </p>
                  </Link>
                </MenuItem>
                <MenuItem>
                  <p
                   onClick={signOut}
                    className="block cursor-pointer px-4 py-2 text-sm text-gray-700 data-[focus]:bg-gray-100 data-[focus]:outline-none"
                  >
                    Sign out
                  </p>
                </MenuItem>
              </MenuItems>
            </Menu>}
          </div>
        </div>
      </div>

      <DisclosurePanel className="sm:hidden">
        <div className="space-y-1 px-2 pb-3 pt-2">
          {navigation.map((item,index) => {
            const list=["My Library","Wishlist"]
            if(!state?.user && list.includes(item.name)){
                return ;
            }
          
           return <Link to={item.href} key={index}> <DisclosureButton
              key={item.name}
              as="a"
              aria-current={item.href== urlPath ? 'page' : undefined}
              className={classNames(
                item.href== urlPath ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white',
                'block rounded-md px-3 py-2 text-base font-medium',
              )}
            >
              {item.name}
            </DisclosureButton>
            </Link>
          })}
        </div>
      </DisclosurePanel>
    </Disclosure>
  )
}
